...
import { useGameWaiting, SYMBOLS } from "@/hooks/useGameWaiting";

/**
 * Game waiting lobby component.
 *
 * This is a pure rendering layer — all state and side-effects live in
 * useGameWaiting so they can be tested independently.
 *
 * TypeScript strict-mode compliant; all nullable paths are explicitly guarded.
 */
export default function GameWaiting(): React.JSX.Element {
  ...
  // --- Invalid code banner (stale / malformed URL param) ---
  if (isInvalidCode) {
    return (
      <section className="w-full min-h-[calc(100dvh-87px)] flex items-center justify-center bg-[#010F10]">
        <div className="space-y-3 text-center bg-[#0A1A1B]/80 p-6 rounded-xl shadow-lg border border-yellow-500/50">
          <p role="alert" className="text-yellow-400 text-lg font-bold font-orbitron">
            Invalid room code in URL.
          </p>
          <p className="text-[#869298] text-sm">
            The code must be exactly 6 alphanumeric characters.
          </p>
    ...
    return (
      <section className="w-full min-h-[calc(100dvh-87px)] flex items-center justify-center bg-[#010F10]">
        <div className="space-y-3 text-center bg-[#0A1A1B]/80 p-6 rounded-xl shadow-lg border border-red-500/50">
          <p role="alert" className="text-red-400 text-lg font-bold font-orbitron animate-pulse">
            {error}
          </p>
          <div className="flex gap-3 justify-center flex-wrap">
            <button
  ...
  const maxPlayersThreshold = gameConfig.maxPlayers;
        ...
        <div className="w-full max-w-xl bg-[#0A1A1B]/80 p-5 sm:p-6 rounded-2xl shadow-2xl border border-[#00F0FF]/50 backdrop-blur-md">
          ...
          <div className="text-center space-y-3 mb-6">
            ...
            <p className="text-yellow-400 text-lg font-bold flex items-center justify-center gap-2 animate-pulse">
              ...
              Entry Stake: {gameConfig.stakeLabel}
            </p>
          ...
          <div className="mb-6 bg-[#010F10]/50 p-4 rounded-xl border border-[#00F0FF]/30 max-h-32 overflow-y-auto">
            ...
            <ul className="space-y-1 text-xs text-[#869298]" aria-live="polite">
              {statusMessages.slice(-5).map((msg) => (
          ...
          <div className="mt-6 space-y-5 bg-[#010F10]/50 p-5 rounded-xl border border-[#00F0FF]/30 shadow-lg">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  disabled={actionLoading || !gameUrl}
                  aria-label="Copy game invite URL"
              ...
              {copySuccess !== null && (
                <p role="status" className="text-green-400 text-xs text-center">{copySuccess}</p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <input
                  type="text"
                  aria-label="Farcaster miniapp invite URL"
                ...
                <button
                  ...
                  aria-label="Copy Farcaster miniapp invite URL"
              ...
              {copySuccessFarcaster !== null && (
                <p role="status" className="text-green-400 text-xs text-center">{copySuccessFarcaster}</p>
              )}
            </div>
            ...
            <div className="flex justify-center gap-5 pt-3">
              ...
              >
                <Share2 className="w-6 h-6" aria-hidden />
              </a>
              <a
              ...
              <div className="flex flex-col bg-[#010F10]/50 p-5 rounded-xl border border-[#00F0FF]/30 shadow-lg">
                ...
                <select
                  ...
                  onChange={(e: React.ChangeEvent<HTMLSelectElement>) => {
                    const selected = SYMBOLS.find((s) => s.value === e.target.value) ?? null;
                    setPlayerSymbol(selected);
                    setError(null);
                  }}
          ...
          <div className="flex justify-between mt-5 px-3 flex-wrap gap-2">
            ...
            >
              <Home className="mr-1 w-4 h-4" aria-hidden /> Back to HQ
            </button>
          </div>
        </div>
      </main>
